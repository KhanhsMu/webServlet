...
public class LoginProcess extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
	    ...
	    try (Connection conn = DBConnect.getConnection()) {
	        String sql = "SELECT idRegis, role FROM registerweb WHERE nameRegis = ? AND passRegis = ?";
	        ...
	        if (rs.next()) {
	            ...
	            if ("admin".equalsIgnoreCase(role)) {
	                response.sendRedirect("ManagerProject"); // Trang admin
	            } else if ("guest".equalsIgnoreCase(role)) {
	                response.sendRedirect("ChooseProject"); // Trang khách
	            } 
	        } else {
