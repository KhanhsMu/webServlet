...
public class ChooseProject extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DBConnect.getConnection()) {
            switch (action) {
                case "register": // Đăng ký đề tài mới
                    ...
                    try (PreparedStatement ps = conn.prepareStatement(sqlRegister)) {
                        ...
                        response.sendRedirect("guestHome.jsp?message=success");
                    } catch (Exception e) {
                        response.sendRedirect("guestHome.jsp?message=error");
                    }
                    break;
            ...
            e.printStackTrace();
            response.getWriter().write("Lỗi khi xử lý yêu cầu: " + e.getMessage());
        }
    }
}
